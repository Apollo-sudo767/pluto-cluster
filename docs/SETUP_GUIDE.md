...
| :--- | :--- | :--- | :--- |
| **`hydra`** | Lenovo ThinkCentre M920q Tiny (i5-8500T, 16GB RAM) | Bootstrap Master (`clusterInit = true`) | `gpu.vendor=intel`, Intel QuickSync transcoding (`/dev/dri`), temporary initial NFS server, secrets sync daemon |
...
hydra   Ready    control-plane,etcd,master   10m   v1.30.x+k3s1
styx    Ready    control-plane,etcd,master   2m    v1.30.x+k3s1
```
...
2. Once Pluto boots and joins `https://hydra:6443`, full 3-node HA quorum is established:
   ...
   ```text
   NAME    STATUS   ROLES                       AGE   VERSION
   hydra   Ready    control-plane,etcd,master   20m   v1.30.x+k3s1
